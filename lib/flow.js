...
function pickRandomModel() {
  const names = selectors.modelNames;
...
async function ensureImageModeAndCount(page, desiredCount = 1, desiredModel = null) {
  ...
  const currentModel =
    [...selectors.modelNames].sort((a, b) => b.length - a.length).find((n) => label.includes(n)) || null;
  ...
  if (inImageMode && countOK && modelOK) {
  ...
  if (!modelOK) {
    const trigger = await page.waitForSelector(selectors.modelDropdown, { timeout: 3000 })
      .catch(() => null);
    if (!trigger) {
      console.log(
        `[flow-daemon] model dropdown trigger NOT FOUND with selector: ${selectors.modelDropdown}`
      );
    } else {
      console.log(`[flow-daemon] model dropdown trigger found — clicking`);
      await trigger.click();
      await humanPause(page, 800, 1600);
      const option = await page.waitForSelector(selectors.modelOption(desiredModel), { timeout: 3000 })
        .catch(() => null);
      if (!option) {
        console.log(
          `[flow-daemon] model option "${desiredModel}" NOT FOUND with selector: ${selectors.modelOption(desiredModel)}`
        );
      } else {
...
async function runJob({ prompt, project_id, segment_id, output_path, rootDir, flowUrl, timeoutMs }) {
  ...
  try {
    ...
    if (!isMockFixture) {
      ...
      await ensureImageModeAndCount(page, 1, chosenModel);
    }
