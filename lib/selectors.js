...
const image = {
  ...
  // "+" button at bottom-left of prompt input — opens the media picker
  // dialog where user can select an existing project image OR upload a
  // new one to attach as a REFERENCE for the image gen (Imagen/Nano Banana
  // image-conditioning). Real Flow text is "add_2Create" (Material icon +
  // label). The selector :not(:has-text("arrow_forward")) excludes the
  // submit-Create button which has same "Create" label but with
  // "arrow_forward" prefix.
  referencePlusButton: 'button:has-text("add_2Create"):not(:has-text("arrow_forward"))',

  // "Upload image" row at bottom of the media picker that opens after
  // clicking the "+" button. Reuses the same text used in Frames mode.
  // Click triggers a Terms popup (first time only) then native filechooser.
  uploadImageOption: 'text=/^Upload image$/i',

  // Same "I agree" Terms popup as Frames mode — first-upload only.
...
const video = {
  ...
  // Count-per-prompt tab inside the mode popover for VIDEO mode.
  // Flow's video mode is inconsistent with image mode: count=1 button
  // is rendered as "1x" (NOT "x1" like image mode). Counts 2-4 use the
  // standard "x2"/"x3"/"x4" form. Verified live 2026-04-30.
  countTab: (n) => n === 1
    ? 'button.flow_tab_slider_trigger:text-is("1x")'
    : `button.flow_tab_slider_trigger:text-is("x${n}")`,
  ...
  // Ingredients-to-video sub-mode tab. Sibling of framesTab inside the
  // popover. Selecting Ingredients lets the user upload up to 3 reference
  // images that Veo composites into the output (max enforced by Flow UI).
  // Mutually exclusive with framesTab — pick one per scene.
  ingredientsTab: 'button.flow_tab_slider_trigger:has-text("Ingredients")',

  // Hidden multi-image file input that accepts the ingredient uploads. Same
  // <input type=file> pattern as Frames upload, but with `multiple` attr so
  // setInputFiles([f1, f2, f3]) works in ONE call instead of three uploads.
  ingredientsFileInput: 'input[type="file"][accept*="image"][multiple]',
