...
function enqueue({ prompt, project_id, segment_id, output_path }) {
  const jobId = newId();
  const job = {
    job_id: jobId,
    status: 'queued',
...
// Find the currently-running job, if any. Returns the full job or null.
// Only one job can be running at a time (enforced by server.js::workerBusy).
function currentJob() {
