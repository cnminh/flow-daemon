...
// Build ffmpeg args for a trim+concat of N clips.
// Clip 0 plays in full; clips 1..N-1 trim `overlap` seconds off the start.
function buildFfmpegArgs(partPaths, overlap, output) {
  ...
  for (let i = 0; i < N; i += 1) {
    if (i === 0) {
      ...
      filters.push(`[${i}:v]trim=start=${overlap},setpts=PTS-STARTPTS[v${i}]`);
...
async function runJob({
  ...
  try {
    ...
    if (partPaths.length === 1) {
      ...
      fs.copyFileSync(partPaths[0], output_path);
      finalSize = fs.statSync(output_path).size;
    } else if (isMockFixture) {
      ...
      // Real Flow, N > 1: run ffmpeg trim+concat.
      const args = buildFfmpegArgs(partPaths, overlapSeconds, output_path);
      console.log(`[flow-video] stitching ${partPaths.length} clips with ffmpeg (overlap ${overlapSeconds}s)`);
      try {
        await runFfmpeg(args);
      } catch (e) {
        const err = new Error(`ffmpeg concat failed: ${e.message}\n${e.stderr || ''}`);
        err.error_code = 'network';
        throw err;
      }
      finalSize = fs.statSync(output_path).size;
