...
async function runJob({
  ...
  try {
    ...
    // Download the final clip. Real Flow's UI has a Download button that
    // opens a resolution-picker modal (270p/720p/1080p/4K — the last costs
    // extra credits), then needs a second click to trigger the actual
    // download. We skip that dance entirely and fetch the <video> element's
    // currentSrc directly — Flow serves mp4 bytes over HTTPS at
    // /fx/api/trpc/media.getMediaUrlRedirect?name=<uuid>. This mirrors how
    // image.js fetches image src after generation.
    //
    // The "last" clip (most-recent <video> we care about) is identified as
    // the newest src that wasn't in beforeSrcs of the final iteration —
    // but at this point `beforeSrcs` is the snapshot from the LAST loop
    // iteration, so the new src is the one we want.
    //
    // Simpler: the most-recently-added <video> with an HTTPS or
    // media-redirect src. Take the first one that isn't a static example
    // clip (the gstatic.com Dolly_in/out etc demos are present on Flow's
    // clip detail view as motion-preset thumbnails).
    //
    // Flow's project-grid view lists ALL clips with the newest first at
    // videos[0]. After a single-clip Create we're already on the grid. But
    // after an extend chain's final Create, we're stuck in the extend-prompt
    // / clip detail state where most <video> elements aren't rendered and
    // the newly-generated clip's currentSrc may not have populated yet.
    // Navigating back to the project URL is idempotent for single-clip runs
    // and fixes the post-extend case.
    if (!isMockFixture) {
    ...
    // Exclude Flow's built-in camera motion demo clips (hosted at
    // gstatic.com) — those are always present on the clip detail page.
    const candidateSrcs = videoSrcs.filter(
      (s) => !s.includes('gstatic.com') && !s.includes('/flow_camera/')
    );
    if (candidateSrcs.length === 0) {
      const err = new Error('no downloadable video src found on the page');
      err.error_code = 'selector_missing';
      throw err;
    }
