...
async function runJob({
  ...
  try {
    ...
    for (let i = 0; i < prompts.length; i += 1) {
      ...
      try {
        ...
        if (!isExtend) {
          ...
          // set. Must run post-ensureVideoMode so the Frames tab is
          // reachable (it lives inside the mode popover, which the ensure
          // helper just closed — uploadFirstFrame re-opens as needed).
          if (frame_path) {
    ...
    if (isMockFixture) {
      ...
      // jobs also get the resolution scale pass. Multi-clip jobs
      // additionally trim the per-seam overlap.
      const args = buildFfmpegArgs(partPaths, overlapSeconds, chosenAspect, output_path, chosenResolution);
      console.log(
...
module.exports = { runJob, pickModelForIndex, buildFfmpegArgs, RESOLUTION_DIMS };
